...
dependencies {
    ...
    komprehensionRuntimeClasspath(project(":runtime"))
...
buildConfig {
    ...
    packageName("com.jsoizo.komprehension.compiler")
    buildConfigField("String", "KOTLIN_PLUGIN_ID", "\"${rootProject.group}\"")
}

tasks.test {
    dependsOn(testArtifacts)
    dependsOn(komprehensionJvmRuntimeClasspath)
    dependsOn(komprehensionJsRuntimeClasspath)

    useJUnitPlatform()
    workingDir = rootDir

    systemProperty("komprehensionRuntime.jvm.classpath", komprehensionJvmRuntimeClasspath.get().asPath)
    systemProperty("komprehensionRuntime.js.classpath", komprehensionJsRuntimeClasspath.get().asPath)

    // The runtime reaches the tests only as system-property paths, so without declaring it as an
    // input the tests stay UP-TO-DATE and pass stale after the runtime changes.
    inputs.files(komprehensionJvmRuntimeClasspath, komprehensionJsRuntimeClasspath)
        .withNormalizer(ClasspathNormalizer::class)
        .withPropertyName("komprehensionRuntime")
...
val generateTests = tasks.register<JavaExec>("generateTests") {
    ...
    mainClass.set("com.jsoizo.komprehension.compiler.GenerateTestsKt")
    workingDir = rootDir
    args(
        listOf(
